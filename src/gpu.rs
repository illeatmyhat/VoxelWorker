...
impl GpuContext {
    ...
    pub async fn new_with_instance(
        ...
        // The GPU brick atlas packs a large scene's covering chunks into one storage buffer
        // before `copy_buffer_to_texture`. A 50×10×50-block cylinder's atlas is ~200 MiB — over
        // the DEFAULT 128 MiB storage-buffer binding limit, which would validation-error the
        // bind group. Raise the storage-buffer and total-buffer size limits to whatever
