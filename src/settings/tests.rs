...
fn old_config_with_removed_keys_still_loads() {
    ...
    let config =
        AppConfig::from_dump_json(old_json).expect("old config with removed keys still parses");
    ...
    assert!(
        panel.scene.master_block_lattice,
        "fresh scene masters default ON"
    );
    assert!(
        panel.scene.master_voxel_grid,
        "fresh scene masters default ON"
    );
    assert!(
        panel.scene.master_floor_grid,
        "fresh scene masters default ON"
    );
...
fn old_config_with_debug_clouds_field_still_loads() {
    ...
    let restored = AppConfig::from_dump_json(old_json)
        .expect("old config (with debug_clouds) must still parse");
    ...
    assert!(
        ...
        "an old flat config carries no scene"
    );
    ...
    let seed_node = panel
        ...
        .roots
        .first()
        .and_then(|id| panel.scene.node_by_id(*id));
...
fn old_config_with_mesher_field_still_loads() {
    ...
    let restored =
        AppConfig::from_dump_json(old_json).expect("old config (with mesher) must still parse");
...
fn full_scene_round_trips_through_json() {
    use document::scene::{DefId, Node, NodeBuilder, NodeContent, NodePath, Scene, VoxelBody};
    use document::voxel::SdfShape;
    ...
    grouped_leaf.transform =
        document::scene::NodeTransform::from_blocks([1, 0, 0], voxels_per_block);
    ...
    let group = NodeBuilder::group_at(
        "Group",
        [6, 0, 0],
        voxels_per_block,
        vec![grouped_leaf.into()],
    );
    ...
    assert_eq!(
        restored_panel.scene.definitions.len(),
        1,
        "the def survives"
    );
    assert_eq!(
...
fn capture_then_to_panel_state_preserves_masters_and_toggles() {
    ...
    assert_eq!(
        restored.scene.master_block_lattice,
        panel.scene.master_block_lattice
    );
    assert_eq!(
        restored.scene.master_voxel_grid,
        panel.scene.master_voxel_grid
    );
    assert_eq!(
        restored.scene.master_floor_grid,
        panel.scene.master_floor_grid
    );
...
fn modern_scene_keeps_its_masters_and_single_origin() {
    ...
    assert_eq!(
        restored_panel.scene.points.len(),
        2,
        "Origin + Marker survive"
    );
}
...
fn unminted_persisted_scene_gets_ids_minted_on_load() {
    use document::scene::{Node, NodeContent, NodeId, NodePath, Scene};
    use document::voxel::SdfShape;
    ...
    *config_value
    ...
    assert!(
        loaded.scene.next_node_id > max_id,
        "counter advanced past every live id"
    );

    // A clicked top-level row resolves to a selectable node (not the sentinel).
    let clicked_id = loaded
        .scene
        .id_at_path(&NodePath::root_index(0))
        .expect("path resolves to an id");
    assert_ne!(clicked_id, NodeId(0));
    assert!(
        loaded.scene.node_by_id(clicked_id).is_some(),
        "the resolved id is selectable"
    );
}
