...
fn a_placed_primitive_tilts_to_the_entered_surface_normal() {
    ...
    let Some(Intent::PlaceNode {
        rotation_quaternion,
        ..
    }) = outcome.intent
    else {
        panic!(
            "a geometry hit produces a PlaceNode, got {:?}",
            outcome.intent
        );
    };
...
impl Fixture {
    ...
    fn apply_and_rebuild(&mut self, intent: Intent) {
        self.app_core
            .apply_intent(&mut self.scene, &mut self.selection, intent);
...
fn a_cursor_over_the_ground_places_a_node_on_it() {
    ...
    let view_projection = fixture
        .app_core
        .view_projection(aspect_ratio, fixture.region_dimensions);
    ...
    assert!(
        matches!(
            ...
            PlacementTarget::OnWorldPlane {
                plane: raycast::WorldPlane::Ground,
                ..
            }
        ),
    ...
    let Some(Intent::PlaceNode { offset_voxels, .. }) = outcome.intent.clone() else {
        panic!(
            "a world-plane hit produces a PlaceNode, got {:?}",
            outcome.intent
        );
    };
    assert_eq!(
...
fn a_geometry_drop_always_seats_to_the_surface() {
    ...
    for angle in [AngleSnap::Continuous, AngleSnap::Deg15] {
        let outcome = fixture.app_core.place_primitive(
            ...
            true,
            PlacementSnap {
                position: PositionSnap::Voxel,
                angle,
                ..PlacementSnap::default()
            },
        );
        let (
            PlacementTarget::OnSurface { face_normal, .. },
            Some(Intent::PlaceNode {
                rotation_quaternion,
                ..
            }),
        ...
        let normal = Vec3::new(
            face_normal[0] as f32,
            face_normal[1] as f32,
            face_normal[2] as f32,
        );
        let axis = Quat::from_array(rotation_quaternion.expect("a seated drop carries the tilt"))
            * Vec3::Z;
        assert!(
...
fn nosnap_deg15_keeps_the_sub_voxel_cursor_position_on_a_flat_face() {
    ...
    let seat = |angle| {
        ...
        assert_eq!(
            face_normal,
            [0, 0, 1],
            "{angle:?}: the top-down ray must enter the flat +Z face"
        );
    ...
    let drift: f32 = (0..3)
        .map(|i| (deg15_pos[i] - continuous_pos[i]).powi(2))
        .sum::<f32>()
        .sqrt();
    assert!(
            drift < 1.0e-4,
...
fn block_snap_rounds_the_offset_to_block_boundaries() {
    ...
    assert_eq!(
        snap_offset([3, 12, -5], PositionSnap::Block, 8),
        [0, 16, -8]
    );
    assert_eq!(
        snap_offset([3, 12, -5], PositionSnap::Voxel, 8),
        [3, 12, -5]
    );
    assert_eq!(
        snap_offset([3, 12, -5], PositionSnap::NoSnap, 8),
        [3, 12, -5]
    );
    // Already block-aligned stays put.
    assert_eq!(
        snap_offset([16, -8, 0], PositionSnap::Block, 8),
        [16, -8, 0]
    );
}
...
fn a_hidden_ground_plane_places_nothing() {
    ...
    assert!(
        ...
        "ground visible ⇒ places on it, got {:?}",
        visible.target
    );
        ...
        for col in 0..8 {
            ...
            assert!(
                !matches!(
                    outcome.target,
                    PlacementTarget::OnWorldPlane {
                        plane: raycast::WorldPlane::VerticalFacingX
                            | raycast::WorldPlane::VerticalFacingY,
                        ..
                    }
                ),
                "a grazing ray placed on an invisible vertical plane: {:?}",
                outcome.target
            );
        }
    }
