...
fn set_operation_on_instance_round_trips() {
    ...
    let mut scene = two_tool_scene();
    scene
        .make_definition_from_node(scene.roots[0], "Body")
        .expect("def made");
...
fn placing_a_subtract_instance_undoes_to_intact_hosts() {
    ...
    assert_eq!(
        scene, after,
        "redo must re-apply the carve placement byte-for-byte"
    );
}
...
fn set_offset_round_trips() {
    ...
    assert_round_trips(
        ...
        Intent::SetOffset {
            target,
            offset_measurements: whole_block_offset([3, -2, 5]),
        },
    );
}
...
fn set_offset_apply_derives_voxels_at_density() {
    ...
    core.apply_intent(
        ...
        Intent::SetOffset {
            target,
            offset_measurements: expression,
        },
    );
    assert_eq!(
...
fn set_offset_undo_restores_retained_measurement() {
    ...
    core.apply_intent(
        ...
        Intent::SetOffset {
            target,
            offset_measurements: first,
        },
    );
    assert_eq!(
        scene.node_by_id(target).unwrap().transform.offset_voxels[0],
        40
    );
...
fn set_density_round_trips() {
    ...
    let mut scene = Scene::from_nodes(vec![
        tool_node(
            SdfShape::from_blocks(ShapeKind::Box, [2, 2, 2], 1, 5),
...
fn set_density_preserves_block_position() {
    ...
    assert_eq!(
        after.transform.blocks(16),
        [5, 0, 0],
        "block 5 preserved across d 8→16"
    );
    assert_eq!(
        after.transform.offset_voxels,
        [80, 0, 0],
        "5 blocks @ d=16 = 80 voxels"
    );
    assert!(
        after.transform.block_aligned(16),
        "still on the mating lattice"
    );

    core.undo(&mut scene, &mut selection);
    let restored = scene.node_by_id(node_id).expect("node survives undo");
    assert_eq!(
        restored.transform.blocks(8),
        [5, 0, 0],
        "block 5 preserved on undo"
    );
    assert_eq!(
        restored.transform.offset_voxels,
        [40, 0, 0],
        "back to 40 voxels @ d=8"
    );
}
...
fn set_offset_undo_across_density_change() {
    ...
    core.apply_intent(
        ...
        Intent::SetOffset {
            target: node_id,
            offset_measurements: whole_block_offset([3, 0, 0]),
        },
    );
    assert_eq!(
...
fn field_write_to_missing_id_undo_is_noop() {
    ...
    core.apply_intent(
        ...
        &mut selection,
        Intent::SetName {
            target: document::scene::NodeId(9999),
...
fn scripted_sequence_undo_redo_round_trips() {
    ...
    let added = selection
        .primary_node_id()
        .expect("AddNode steers onto the new node");
    core.apply_intent(
        &mut scene,
        &mut selection,
        Intent::GroupNode { target: added },
    );
    // The wrapped child is now the primary selection; group IT into a definition.
    let active = selection
        .primary_node_id()
        .expect("GroupNode steers onto the child");
    core.apply_intent(
        &mut scene,
        &mut selection,
        Intent::MakeDefinition {
    ...
    core.apply_intent(
        ...
        Intent::SetOffset {
            target: instance,
            offset_measurements: whole_block_offset([7, 0, 0]),
        },
    );
    core.apply_intent(
        &mut scene,
        &mut selection,
        Intent::RemoveNode { target: instance },
    );
    ...
    assert_eq!(
        scene, seed,
        "undo all the way restores the seed byte-for-byte"
    );
    ...
    assert_eq!(
        scene, final_scene,
        "redo all the way restores the final scene"
    );
}
...
fn undo_of_field_edit_reports_scene_not_points() {
    ...
    assert!(
        !redo_effect.points_changed,
        "redo of a rename does not touch points"
    );
}
...
fn undo_of_grid_masters_does_not_claim_scene_changed() {
    ...
    assert!(
        !undo_effect.scene_changed,
        "grid masters need no re-resolve"
    );
    assert!(
        !undo_effect.points_changed,
    ...
    assert!(
        ...
        "redo of grid masters needs no re-resolve"
    );
}
...
fn rebuild_yields_sparse_two_layer_output_no_dense_grid() {
    ...
    assert!(
        scene.has_chunkable_extent(density),
        "the two-tool fixture is chunkable"
    );
    let mut core = test_core();
...
fn toggling_voxel_grid_on_faces_appears_on_first_rebuild() {
    ...
    assert_eq!(
        before, 0,
        "with the flag OFF no voxel may carry the grid_overlay marker"
    );
