//! The undo/redo stacks — workspace state, not document state.
//!
//! Undo/redo is a **linear stack of inverse commands**. [`document::command`] owns the
//! pieces that need `Scene` internals: the forward [`Intent`](document::intent::Intent) and
//! its [`Inverse`](document::command::Inverse). Which stack a command sits on and what order
//! transactions reverse in is runtime state belonging to the shell's `AppCore`, so the
//! stacks live here.
//!
//! The stacks hold [`Command`]s bare: no selection rides along. Undo touches only the
//! document, and a validity prune after every mutation is what keeps the selection honest.

use document::command::Command;

/// One atomic undo step on the main stack — a **transaction** of one or more
/// [`Command`]s applied together and reversed together. A normal
...
/// The linear undo/redo command stack: two Vecs, no branching.
...
pub struct CommandStack {
    ...
    /// The OPEN sketch-editing group, or `None` outside sketch mode.
    ///
    ...
    /// AND counter) and discards it. One concept, one stack, no parallel history: the
    /// session is a scoped detour on the same machinery.
    ///
...
/// The transient history of ONE sketch-editing session — non-document, like all
...
pub struct SketchGroup {
    ...
    ///
    /// Transactions and not bare [`Command`]s because ONE authoring act is one in-mode undo
    /// step: a click that both places a vertex and re-anchors the node emits two
