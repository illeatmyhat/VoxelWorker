...
use document::scene::{
    DefId, Node, NodeBuilder, NodeContent, NodeGrids, NodeId, NodeTransform, Point, Scene,
    VoxelBody, COORDINATE_LIMIT_BLOCKS,
};
...
fn offset_just_inside_coordinate_limit_is_applied() {
    ...
    let effect = core.apply_intent(
        ...
        Intent::SetOffset {
            target,
            offset_measurements: whole_block_offset([inside, 0, 0]),
        },
    );
    ...
    assert_eq!(
        core.undo_depth(),
        1,
        "an accepted edit records one undo step"
    );
    assert_eq!(
...
fn offset_beyond_coordinate_limit_is_rejected() {
    ...
    let effect = core.apply_intent(
        ...
        Intent::SetOffset {
            target,
            offset_measurements: whole_block_offset([beyond, 0, 0]),
        },
    );
    assert_eq!(applied, scene, "a rejected edit leaves the scene untouched");
    assert!(
        effect.coordinate_limit_rejected,
        "the effect flags the rejection"
    );
...
fn place_node_beyond_coordinate_limit_is_rejected() {
    ...
    assert_eq!(
        applied, scene,
        "a rejected placement leaves the scene untouched"
    );
...
fn rejection_flags_the_effect_then_an_accepted_edit_clears_it() {
    ...
    let accepted = core.apply_intent(
        ...
        Intent::SetOffset {
            target,
            offset_measurements: whole_block_offset([1, 0, 0]),
        },
    );
...
fn every_intent_variant_round_trips_through_json() {
    ...
    let variants = vec![
        ...
        Intent::SetOffset {
            target: NodeId(1),
            offset_measurements: whole_block_offset([-1, 2, -3]),
        },
        Intent::SetName {
            target: NodeId(1),
            name: "Foo".to_string(),
