...
fn targeted_invalidation_evicts_only_intersecting_chunks() {
    ...
    scene_b.root_node_mut(1).transform =
        document::scene::NodeTransform::from_blocks([80, 0, 0], density);
    ...
    assert!(
        !expected_evicted.is_empty(),
        "the move must dirty at least one resident chunk"
    );
    ...
    for coord in &expected_evicted {
        assert!(
            !after.contains(coord),
            "chunk {coord:?} intersecting the edit must be evicted"
        );
    }
    for coord in &all_resident {
        if !expected_evicted.contains(coord) {
            assert!(
                after.contains(coord),
                "chunk {coord:?} outside the edit must stay resident"
            );
        }
    }
...
fn move_invalidates_chunks_around_both_endpoints() {
    ...
    assert!(
        min_chunk[0] <= old_chunk_x && old_chunk_x <= max_chunk[0],
        "edit range must cover OLD chunk"
    );
    assert!(
        min_chunk[0] <= new_chunk_x && new_chunk_x <= max_chunk[0],
        "edit range must cover NEW chunk"
    );
}
...
fn empty_edit_aabb_evicts_nothing() {
    ...
    assert!(
        evicted.is_empty(),
        "an empty edit AABB reports an empty evicted set"
    );
    assert_eq!(
        resident_coords(&cache),
        before,
        "an empty edit AABB evicts nothing"
    );
}
...
fn invalidate_aabb_returns_exactly_the_evicted_coords() {
    ...
    let returned: std::collections::BTreeSet<[i32; 3]> = cache
        .invalidate_aabb(&edit_aabb, density)
        .into_iter()
        .collect();
    assert_eq!(
        returned, expected,
    ...
    for coord in &returned {
        assert!(
            !after.contains(coord),
            "a returned coord must no longer be resident"
        );
    }
    assert_eq!(
...
fn invalidate_aabb_density_mismatch_reports_all_resident_evicted() {
    ...
    assert_eq!(
        returned, before,
        "a density mismatch evicts (and reports) every resident coord"
    );
