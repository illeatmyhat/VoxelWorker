...
fn spilled_and_reloaded_chunk_is_byte_identical() {
    ...
    assert!(
        coords.len() >= 2,
        "need at least two covering chunks to force a spill"
    );
    ...
    let expected: std::collections::HashMap<[i32; 3], _> = coords
        ...
        .map(|&coord| {
            (
                coord,
                occupied_multiset(reference.chunk(coord, &scene, density, 0)),
            )
        })
        .collect();
    ...
    for &coord in &coords {
        ...
        assert!(
            cache.resident_chunk_count() <= 1,
            "capacity 1 keeps at most one resident"
        );
    }
    assert!(
        cache.spill_count() >= 1,
        "filling past capacity 1 must spill"
    );
...
fn resident_cap_is_never_exceeded() {
    ...
    assert!(
        coords.len() > cap,
        "the scene must have more chunks than the cap to exercise spill"
    );
...
fn invalidation_purges_resident_and_disk() {
    ...
    assert!(
        cache.spill_count() >= 1,
        "coords[0] must be spilled to disk"
    );
    let reloads_before = cache.disk_reload_count();
    ...
    assert!(
        cache2.spill_count() >= 1,
        "resolve_region over cap 1 must spill"
    );
...
fn unbounded_cache_never_spills() {
    ...
    assert!(
        cache.resident_chunk_count() > 1,
        "an unbounded cache keeps every chunk resident"
    );
    assert_eq!(cache.spill_count(), 0);
    assert_eq!(cache.disk_reload_count(), 0);
    assert!(
        cache.recompute_count() > 0,
        "recompute count tracks first-time resolves"
    );
}
...
fn zero_resident_cap_panics() {
    ...
    let result = std::panic::catch_unwind(|| Store::with_resident_cap(0, &temp.path));
