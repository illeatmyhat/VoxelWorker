...
fn resolved_occupancy_set(grid: &VoxelGrid) -> std::collections::BTreeMap<[u32; 3], u16> {
...
fn sketch_interior_elides_to_coarse_solid() {
    ...
    assert!(
        coarse > 0,
        "full 360 revolve must elide interior blocks to coarse-solid"
    );
    ...
    assert!(
        coarse > 0,
        "the L extrude must still elide its solid interior"
    );
    ...
    assert!(
        coarse > 0,
        "the triangle extrude must still elide its solid interior"
    );
    ...
    let box_scene = Scene::from_nodes(vec![Node::new(
        ...
        NodeContent::SketchTool {
            producer: SketchSolid::extrude(
                Sketch::rectangle(PlaneAxis::Z, edge, edge),
                edge as u32,
            ),
            material: MaterialChoice::Stone,
        },
        ...
        for chunk_y in min_chunk[1]..=max_chunk[1] {
            for chunk_x in min_chunk[0]..=max_chunk[0] {
                ...
                let naive: Vec<usize> = leaves
                    .iter()
                    .enumerate()
                    .filter(|(_, leaf)| leaf_world_aabb(leaf, density).intersects(&chunk_box))
                    .map(|(index, _)| index)
                    .collect();
                assert_eq!(
