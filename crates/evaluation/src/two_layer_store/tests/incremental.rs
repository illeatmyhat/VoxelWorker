...
fn full_two_layer_resident(scene: &Scene, density: u32) -> BTreeMap<[i32; 3], TwoLayerChunk> {
...
fn incremental_two_layer_equals_full_rebuild_for_every_edit_kind() {
    ...
    let resize = {
        let mut b = scene_a.clone();
        let replacement = incr_tool_node(
            ShapeKind::Box,
            [3, 3, 3],
            [60, 0, 0],
            MaterialChoice::Wood,
...
fn widest_run_scaling_probe() {
    ...
    for blocks in [50i64, 125, 250, 500] {
        ...
        let widest =
            streamed_widest_run_in_band(&TwoLayerStore::enabled(), &scene, density, 0, edge as u32);
...
fn two_layer_sketch_box_build_probe() {
    ...
    for blocks in [25i64, 50] {
        ...
        let coarse: u64 = chunks
            ...
            .sum();
        let sculpted: u64 = chunks
            .iter()
            .map(|(_, chunk)| chunk.microblocks.len() as u64)
            .sum();
        println!(
...
fn incremental_two_layer_localized_recolor_evicts_few_chunks() {
    ...
    assert!(
        total >= 8,
        "the spread scene has many resident chunks ({total})"
    );
    assert!(
        evicted * 2 < total,
...
fn incremental_two_layer_move_clears_source_and_fills_destination() {
    ...
    assert!(
        evicted < total,
        "a move touches a strict subset ({evicted} of {total})"
    );
}
...
fn incremental_two_layer_density_change_falls_back_to_wholesale() {
    let density_a = 16u32;
    let density_b = 8u32;
    let scene = Scene::from_nodes(vec![incr_tool_node(
        ShapeKind::Sphere,
        [5, 5, 5],
        [0, 0, 0],
        MaterialChoice::Stone,
