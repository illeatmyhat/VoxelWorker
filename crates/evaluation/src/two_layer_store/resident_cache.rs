...
/// **The resident two-layer display cache.**
///
...
/// **only the chunks an edit's world-AABB intersects** (chunk-granular),
...
/// sufficient to retire the dense path. A **block-granular dirty-brick recompute** —
/// re-classify only the blocks the edit AABB touches, keeping the rest of the chunk's coarse
/// layer — is an available optimization, not what this does.
///
/// ## Frame — why a recenter shift does NOT invalidate the cache
///
...
pub struct TwoLayerResidentCache {
    /// The two-layer capability flag, forwarded to the stateless builder.
    ...
    enabled: bool,
    /// Resident chunks keyed by ABSOLUTE chunk coord (the only LOD in use is 0).
    ///
...
impl TwoLayerResidentCache {
    ...
    /// **Targeted invalidation, mirroring
    ...
    /// voxels, producer-true frame), at `voxels_per_block` — whole-chunk
    ...
    pub fn resident_two_layer_chunks(
        ...
        //: gather the missing coords, build them in parallel into a Vec, THEN insert
        // serially (the insert is cheap next to the build). This keeps the incremental
        // dirty-set path intact — only chunks actually absent are (re)built, resident
        ...
        //
        // HOIST the leaf list out of the per-chunk build (compute ONCE, not per missing
        // chunk); the EDIT BROADPHASE — a stateless per-build BVH
        ...
        // resident HITs are reused verbatim (the dirty-set path is intact).
