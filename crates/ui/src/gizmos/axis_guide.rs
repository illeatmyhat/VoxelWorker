...
/// A **snap axis guide** — the dashed extension line in the [`Axis`] color (X `#d9603f` /
/// Y `#7dba6a` / Z accent) that a vertex snapped to an in-plane axis runs along. Axis-lock and
/// equal length fall out of the same lattice, so the guide's color IS the constraint it stands
/// in for — the by-product a solver would call an alignment constraint.
