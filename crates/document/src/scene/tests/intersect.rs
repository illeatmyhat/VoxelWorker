...
fn intersect_after_body_keeps_only_the_overlap() {
    ...
    for (index, material) in survivors.keys() {
        let inside_overlap =
            (0..3).all(|axis| index[axis] >= overlap_low && index[axis] < overlap_high);
        assert!(
            inside_overlap,
...
fn intersect_respects_sealed_scopes() {
    ...
    let mut scene = Scene::from_nodes(vec![
        ...
        NodeBuilder::group(
            "Mask scope",
            vec![box_tool([2, 2, 2], [2, 2, 2], MaterialChoice::Wood, CombineOp::Union).into()],
        ),
    ]);
...
fn chunked_resolve_matches_monolithic_for_intersect_scenes() {
    ...
    let mut scene = Scene::from_nodes(vec![
        ...
        NodeBuilder::group(
            "Mask scope",
            vec![box_tool([2, 2, 2], [5, 5, 5], MaterialChoice::Wood, CombineOp::Union).into()],
        ),
    ]);
    let group_id = scene.roots[1];
    scene
