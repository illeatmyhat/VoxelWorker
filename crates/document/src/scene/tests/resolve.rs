...
fn chunked_resolve_matches_monolithic_for_sketch_extrude() {
    ...
    let l_extrude = SketchSolid::extrude(Sketch::new(PlaneAxis::Z, l_profile), 3 * density as u32);
...
fn outset_grows_a_box_by_the_outset_on_every_side() {
    ...
    let resolved_side = |outset_voxels: i64| {
        let shape = SdfShape::from_blocks(ShapeKind::Box, [size_blocks; 3], 1, voxels_per_block);
        let mut node = Node::new(
            "Box",
            NodeContent::Tool {
                shape,
    ...
    assert_eq!(
        resolved_side(0),
        side as i64,
        "an outset of 0 must not change the body"
    );
    assert_eq!(
        resolved_side(4),
        side as i64 + 8,
...
fn an_outset_on_a_part_dilates_the_parts_composed_body() {
    ...
    assert_eq!(
        plain,
        32 * 32 * 32,
        "the undilated Part is a plain 4-block box"
    );
    assert_eq!(
        dilated,
        40 * 40 * 40,
...
fn an_outset_shell_takes_the_nearest_members_material() {
    ...
    let stone_count = grid
        .occupied
        .iter()
        .filter(|v| v.block_id == stone_id)
        .count();
    let wood_count = grid
        .occupied
        .iter()
        .filter(|v| v.block_id == wood_id)
        .count();
