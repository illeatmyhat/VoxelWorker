...
fn from_measurements_integer_multiple_density_keeps_voxel_term_exact() {
    ...
    let at_thirty_two = NodeTransform::from_measurements(at_sixteen.offset_measurements(), 32);
    assert_eq!(
...
fn mixed_tool_and_cloud_resolve_in_one_frame() {
    ...
    let cloud = Node::new(
        "Clouds",
        NodeContent::VoxelBody(VoxelBody::DebugClouds { seed: 3 }),
    );
    let scene = with_minted_ids(Scene::from_nodes(vec![tool, cloud]));
...
fn two_node_scene_resolves_to_union() {
    ...
    let sphere_only =
        Scene::single_node(sphere.clone()).resolve_region(region, voxels_per_block, 0);
    let cube_only = Scene::single_node(cube.clone()).resolve_region(region, voxels_per_block, 0);
...
fn wood_tool_stamps_wood_material_id() {
    ...
    assert!(
        grid.occupied
            .iter()
            .all(|voxel| voxel.color_index() == wood_id),
...
fn two_material_scene_has_both_material_ids() {
    ...
    assert!(
        grid.occupied
            .iter()
            .any(|voxel| voxel.color_index() == stone_id),
        "the Stone node's voxels must carry the Stone id"
    );
    assert!(
        grid.occupied
            .iter()
            .any(|voxel| voxel.color_index() == wood_id),
...
fn voxel_grid_flag_bit_set_iff_node_opts_in() {
    for &voxels_per_block in &[1u32, 15, 16] {
        ...
        assert!(
            grid.occupied.iter().all(|v| v.grid_overlay),
...
fn voxel_grid_flag_bit_is_per_object() {
    ...
    assert!(
        stone_flagged,
        "the enabled (Stone) node's voxels must all be flagged"
    );
    assert!(
        wood_unflagged,
        "the disabled (Wood) node's voxels must all be unflagged"
    );
    assert!(
...
fn offset_node_shifts_voxels_by_blocks_times_density() {
    ...
    assert!(
        !cluster_low.is_empty() && !cluster_high.is_empty(),
        "both boxes present"
    );
    assert_eq!(
        cluster_low.len(),
        cluster_high.len(),
        "both boxes fill one block"
    );
    ...
    let shifted: std::collections::HashSet<[i64; 3]> = cluster_low
        ...
        .collect();
    assert_eq!(
        shifted, cluster_high,
        "offset N blocks shifts voxels by exactly N×density"
    );
}
...
fn disjoint_offsets_give_summed_occupancy() {
    ...
    assert_eq!(
        region.size_blocks,
        [6, 1, 1],
        "composite extent encompasses both offsets"
    );
    let grid = scene.resolve_region(region, voxels_per_block, 0);
    assert_eq!(
        grid.occupied_count(),
...
fn full_extent_encompasses_offset_node() {
    ...
    assert_eq!(
        single.size_blocks,
        [2, 2, 2],
        "a lone offset box keeps its own size"
    );
...
fn nested_group_composes_transforms_down() {
    ...
    let mut leaf = Node::new(
        "Leaf",
        NodeContent::Tool {
            shape: unit_box_shape(),
            material: MaterialChoice::Stone,
        },
    );
    leaf.transform = NodeTransform::from_blocks([b, 0, 0], voxels_per_block);
    let grouped = with_minted_ids(Scene::from_nodes(vec![NodeBuilder::group_at(
        "Group",
        [a, 0, 0],
        voxels_per_block,
        vec![leaf.into()],
    ...
    assert!(
        grouped_grid.occupied_count() > 0,
        "the grouped leaf must emit voxels"
    );
    assert_eq!(
...
fn instance_matches_direct_placement() {
    ...
    assert!(
        instanced_grid.occupied_count() > 0,
        "the instance must emit voxels"
    );
    assert_eq!(
...
fn two_instance_village_doubles_occupancy_disjointly() {
    ...
    let mut def_only_scene = Scene::from_nodes(vec![Node::new("I", NodeContent::Instance(def_id))]);
    ...
    assert!(
        low.is_disjoint(&high),
        "the two houses occupy disjoint locations"
    );
}
...
fn self_referential_definition_does_not_overflow() {
    ...
    let mut scene_build = Scene::from_nodes(vec![Node::new("Root", NodeContent::Instance(def_id))]);
...
fn wrap_node_in_group_nests_node_under_new_group() {
    ...
    let node_a_id = scene
        .id_at_path(&NodePath::root_index(0))
        .expect("A has an id");
    ...
    match &scene.root_node(0).content {
        NodeContent::Group(children) => {
            assert_eq!(
                children.len(),
                1,
                "the Group holds exactly the wrapped node"
            );
            assert_eq!(
...
fn make_definition_creates_def_and_instance() {
    ...
    assert_eq!(
        ...
        1,
        "a definition appears in scene.definitions"
    );
    let def = scene
        .def_by_id(def_id)
        .expect("the new def is looked up by id");
    assert_eq!(def.name, "House");
    assert_eq!(
        def.children.len(),
        1,
        "a single leaf becomes a one-node body"
    );
    ...
    assert_eq!(
        after, before,
        "an instance of the def equals the original node"
    );
}
...
fn add_instance_multiplies_occupancy_via_one_definition() {
    ...
    assert_eq!(
        scene.roots.len(),
        1,
        "the original node became one instance"
    );
    ...
    assert_eq!(
        scene.roots.len(),
        2,
        "an Instance node referencing the def appears"
    );
    assert!(matches!(
    ...
    assert_eq!(
        scene.definitions.len(),
        1,
        "the body is stored once, not copied"
    );
    ...
    assert_eq!(
        total,
        2 * one,
        "two instances of one def → 2× the def's voxel count"
    );
}
...
fn tree_rows_lists_group_children_indented() {
    ...
    //   [1]          B                depth 0
    let group_id = scene
        .wrap_node_in_group(scene.roots[0])
        .expect("A resolves");
    ...
    assert!(
        added.is_some(),
        "the wrapped node is a Group so a child can be added"
    );
    ...
    assert_eq!(
        rows[0].1,
        crate::scene::ROOT_NODE_ID,
        "row 0 is the root part"
    );
}
...
fn node_at_path_reaches_group_child() {
    ...
    let node = scene
        ...
        .expect("the child resolves by path");
