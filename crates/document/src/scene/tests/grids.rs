...
fn scene_default_master_grids() {
    ...
    assert!(
        scene.master_block_lattice,
        "block lattice master defaults ON"
    );
    ...
    assert!(
        ...
        "no Points until ensure_origin_point"
    );
}
...
fn ensure_node_ids_mints_unique_stable_ids() {
    ...
    assert!(
        ids.iter().all(|&id| id != NodeId(0)),
        "no node keeps the 0 sentinel"
    );
...
fn ensure_node_ids_preserves_existing_and_advances_counter() {
    ...
    assert!(
        scene.arena.contains_key(&NodeId(5)),
    ...
    assert!(
        !scene.arena.contains_key(&NodeId(0)),
        "the 0 sentinel is gone"
    );
    let fresh_id = scene
        .arena
        .iter()
    ...
    assert_eq!(
        scene.roots[1], fresh_id,
        "the root spine slot was repointed off the sentinel"
    );
    assert_eq!(
        scene
            .node_at_path(&NodePath::root_index(1))
...
fn node_id_and_path_resolution_round_trip() {
    ...
    assert_eq!(
        scene.node_at_path(&NodePath::root_index(0)).unwrap().name,
        "renamed"
    );
}
...
fn add_point_defaults_planes_off_axes_on() {
    ...
    assert!(
        !point.plane_xz && !point.plane_xy && !point.plane_yz,
        "new point: all planes OFF"
    );
    assert!(
        point.axis_x && point.axis_y && point.axis_z,
        "new point: all axes ON"
    );
    ...
    assert!(
        origin_scene.points[0].plane_xy,
        "Origin keeps the ground plane (XY)"
    );
}
...
fn old_scene_json_loads_with_grid_defaults() {
    ...
    assert_eq!(
        scene.root_node(0).grids,
        NodeGrids::default(),
        "grids default off"
    );
    assert!(scene.master_block_lattice, "lattice master default on");
    assert!(
        scene.master_voxel_grid && scene.master_floor_grid,
        "all masters default on"
    );
