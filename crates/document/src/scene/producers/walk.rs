...
//! ([`Scene::composed_scope_leaf`] and friends) a scope outset requires.

use super::*;
use crate::scene::*;

impl Scene {
    /// Walk the whole node tree depth-first, invoking `visitor` once for every **enabled
    /// leaf** (`Tool` / `VoxelBody`) with its accumulated **world** VOXEL offset
    /// (`parent_offset + node.offset_voxels`, summed down the tree — translation-only
    /// composition, in voxels at the document density).
    ///
    ...
    ///
    /// The visitor receives, per enabled leaf: its accumulated world VOXEL offset, its
    /// content, its own `grids.voxel_grid_on_faces` flag (the resolver stamps it as each
    /// voxel's dedicated `grid_overlay` attribute, so the on-face voxel grid travels with
    /// each voxel through chunk bucketing), its own [`CombineOp`] (the leaf's role in the
    /// ordered document-order fold — a `Subtract` leaf removes occupancy from everything
    /// accumulated before it *in its scope*), and its **scope path** — the chain of
    /// enclosing sealed composition scopes: every `Group` node and every
    /// `Instance`-expanded SEALED definition body on the way down, outermost first, each
    /// carrying the SCOPE node's own operation (see [`ScopeFrame`]). A scope pre-composes
    /// its leaves into one body; a boolean inside it can never affect geometry outside it.
    /// A FIXTURE definition's expansion contributes NO frame: its leaves splice into the
    /// hosting scope's fold carrying the HOST's path, so they compose directly at the
    /// instance's position.
    ///
    ...
    pub(crate) fn for_each_leaf(&self, visitor: &mut LeafVisitor<'_>) {
        ...
        // The ROOT PART is a scope too (a field rather than an arena entry, but still the
        // container the top-level nodes compose in). So it pre-composes on the same two
        // triggers a Group does: its own outset dilates the whole scene, and a top-level
        // Emboss needs the accumulated field its siblings form.
        //
            ...
            self.composed_scope_leaf(&self.root, &self.roots, [0, 0, 0], &mut def_path, None)
        {
            visitor(VisitedLeaf {
                ...
                // The root composite sits at its integer origin with no continuous slide of
                // its own (its members' float offsets are baked into the composed producer,
                // not carried here), and carries no rotation — so it is upright.
    ...
    /// Collect every enabled leaf as a [`LeafProducer`]: its world voxel offset, a boxed
    /// [`VoxelProducer`](crate::voxel::VoxelProducer), and its single-material override id.
    /// This is the op-stack the two-layer classifier / boundary-resolve evaluate over — the
    /// SAME leaves [`resolve_chunk_rebased`](Self::resolve_chunk_rebased) stamps, in the SAME
    ...
    pub fn leaf_producers(&self, voxels_per_block: u32) -> Vec<LeafProducer> {
        ...
        self.for_each_leaf(&mut |VisitedLeaf {
            ...
            // The outset dilates the body BEFORE it folds. Wrapping the producer (rather
            // than teaching the fold a new arm) means the classifier's `cell_field_interval`
            // call and the voxel-set fold both see one definition of what outset means —
            // see `OutsetProducer`.
            ...
            leaves.push(LeafProducer {
                ...
                // and the frame is carried, never re-derived.
                world_offset_voxels: std::array::from_fn(|axis| {
                    world_offset_voxels[axis] - outset_voxels
                }),
                // The continuous rotation and the float local offset. The `- outset_voxels`
                // frame adjustment is an INTEGER-frame move only — the float slide is
                // relative to the (unadjusted) integer origin, so it is NOT re-based here.
                rotation,
                offset_local_voxels,
                producer,
                material,
    ...
    /// of its UNION members (the only ones that can bound the body — voxel-valued, so
    /// density-independent), rebase every member onto it, and wrap it in a
    /// [`ComposedScope`]. `None` when there are no members, or no union member (the scope
    /// composes to nothing it could dilate). One definition of how a composite's origin is
    /// derived; the two callers differ only in the fingerprint.
    fn finish_composite(
    ...
    /// Part containing one could not be outset even if it were composed. Declining leaves
    /// such a Part walking as an ordinary scope.
    fn composed_scope_leaf(
        ...
        // Two reasons to pre-compose: the scope is DILATED as a whole, or one of its members
        // EMBOSSES and so needs the accumulated body as a field rather than as a voxel set.
    ...
    /// a FIXTURE definition splices its children inline, and the cycle guard is the same.
    /// `None` aborts the whole composition (see
        ...
        for &node_id in spine {
            ...
            match &node.content {
                ...
                NodeContent::Instance(def_id) => {
                    ...
                    // The OUTERMOST instance wins, so an already-set host is kept — a
                    // definition containing another instance still redirects a pick to the
                    // placement the user can see.
                    let inner_host = instance_host.or(Some(node.id));
                    let outcome = if def.fixture {
                        // A fixture does NOT pre-compose — its children splice into the
                        // hosting scope's fold under their own operations.
                        self.collect_composite_members(
                            &def.children,
                            world_offset_voxels,
                            def_path,
    ...
    /// integer world VOXEL offset plus the continuous float slide relative to it, carried
    /// together and handed to the visitor; `instance_host` is the outermost `Instance` this
    /// spine is being expanded under, or `None` in the scene proper — what a viewport pick
    /// redirects to; `def_path` is the stack of definition ids currently being expanded (for
    /// the cycle guard — an `Instance` that would re-enter a definition already on the path
    /// is skipped instead of recursing forever); `scope_path` is the stack of enclosing
    /// sealed-scope frames (pushed on entering a `Group` or an `Instance`'s definition body,
    /// popped on leaving) handed to the visitor per leaf.
    pub(crate) fn walk_nodes(
        ...
        // GOLDEN-CRITICAL: iterate the id-spine for ORDER (document order = later-wins on
        // overlap), fetching each node's content from the arena. NEVER iterate the arena to
        // produce this walk — that visits in id order and would reorder Union material on
        // overlap, moving the goldens.
        for &node_id in spine {
            let Some(node) = self.arena.get(&node_id) else {
                continue;
            };
            ...
            // The integer offset and the continuous float slide accumulate the same way,
            // additively, and travel together to the visitor.
            ...
            match &node.content {
                ...
                | NodeContent::VoxelBody(_) => {
                    // The leaf carries its OWN `operation` plus the chain of enclosing
                    // sealed-scope frames into the flat walk — consumers reconstruct
                    // the scoped fold from the paths.
                    visitor(VisitedLeaf {
                        world_offset_voxels,
                        offset_local_voxels: world_offset.local_voxels,
                        // The leaf's continuous rotation, the whole tilt seated against the
                        // surface it was dropped on. The classifier reads this quaternion
                        // directly (a lattice turn is just a rotation on the exact
                        // classifier path).
                ...
                NodeContent::Group(children) => {
                    // An outset on a SCOPE dilates the scope's COMPOSED body, so the scope
                    // is evaluated as one producer and handed to the visitor as a single
                    // leaf rather than recursed into. Per-member dilation is a different
                    // operation and is not what an outset means: it would make an internal
                    // Subtract cutter carve MORE, where dilating the composed Part grows
                    // the finished body and partly closes that cut.
                    if let Some(composed) = self.composed_scope_leaf(
                        ...
                        visitor(VisitedLeaf {
                            world_offset_voxels: composed.origin_voxels,
                            // A composed scope carries no continuous slide of its own
                    ...
                    // A Group is a SEALED composition scope — its frame (identity +
                    // the GROUP node's own operation) encloses every leaf below it,
                    // so the group's children pre-compose into one body that folds
                    // into the parent under the group's operation.
                ...
                NodeContent::Instance(def_id) => {
                    ...
                    def_path.push(*def_id);
                    // The OUTERMOST instance wins — set on first entry and kept through
                    // any nested instance, so a pick redirects to the placement the user
                    // can actually see rather than one buried in a definition.
                    let inner_host = instance_host.or(Some(node.id));
                    if def.fixture {
                        // A FIXTURE definition does NOT pre-compose — NO scope frame
                        // is pushed, so its children SPLICE into the HOSTING scope's
                        // fold at this instance's spine position, in order, under the
                        // instance's transform (`world_offset_voxels` composes exactly
                        // as for a sealed body — the frame is carried; the host is
                        // POSITIONAL, never a stored reference). The instance's
                        ...
                        // A definition body is a SEALED scope — it pre-composes
                        // (internal booleans are fully
