...
impl Scene {
    ...
    /// `pub` so the chunk cache (up in the app crate) iterates the covering chunks for
    /// reassembly.
    ///
    ...
    pub fn covering_chunk_range(&self, voxels_per_block: u32) -> Option<([i32; 3], [i32; 3])> {
        ...
        // blocks at density 16 a chunk coord is ≤ ±2.5×10⁸, well inside i32.
    ...
    /// Build a [`LeafSpatialIndex`] over the scene's leaves at `voxels_per_block`.
    ///
    ...
    pub fn build_leaf_spatial_index(&self, voxels_per_block: u32) -> LeafSpatialIndex {
        ...
        self.for_each_leaf(&mut |VisitedLeaf {
            ...
            // The edit-broadphase AABB must be the OUTSET bounds, not the producer bounds —
            // an outset cutter dirties a larger region than its own extent, and invalidating
            // only the undilated box leaves a stale rim behind.
            ...
            match body.grid_voxels(voxels_per_block, outset_voxels) {
                Some(grid_voxels) => {
                    // The leaf's world box is its TURNED grid, corner-anchored at its world
                    // offset — so the broadphase covers the axes it actually occupies.
                    ...
                    // An Intersect-influence leaf's edits cannot be localized to its box
...
///
/// The absolute-VOXEL math is i64 so a far-placed node composes without overflow, but the
/// CHUNK coordinate (= voxel / chunk_extent) is much smaller — at density 16 /
/// `CHUNK_BLOCKS = 4` the extent is 64 voxels, so a block offset of ±10⁹ yields a chunk
/// coord of only ±2.5×10⁸, comfortably inside i32 (±2.1×10⁹). Keeping the chunk coord /
/// cache key i32 is therefore safe and avoids widening the whole chunk index. A coordinate
/// that would not fit i32 means a block offset past ~±8×10⁹ and is clamped (debug-asserted)
/// rather than silently wrapping.
///
/// **±8×10⁹ is NOT the supported placement range.** That bound covers the chunk coordinate,
/// which is a voxel index DIVIDED by the chunk extent, and the two-layer expansion
/// multiplies back by that extent to rebase each voxel into an `i32` `local_index`.
/// Bounding the quotient says nothing about the product: at ±8×10⁹ blocks the voxel index
/// overruns `i32` by 4× at density 1 and 238× at density 64. The real range is
/// [`voxel_core::core_geom::max_supported_block_offset`] (±3.3×10⁷ blocks at density 64),
/// which is what the expansion's unchecked `as i32` actually tolerates — Kani-proved there.
fn narrow_chunk_coord(chunk_coord: i64) -> i32 {
    debug_assert!(
        ...
        "chunk coordinate {chunk_coord} overflows i32 — block offset is past the \
         ±~8×10⁹-block range an i32 chunk key can address"
    );
