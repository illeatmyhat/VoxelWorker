//! The per-node transform gizmo's recentred placement (issue #29 S2): the pivot
//! (centre of the node subtree's block-aligned AABB) and its extent.

use super::*;

impl Scene {
    /// The transform gizmo's placement for `node_id`, in the SAME recentred render
    ...
    /// Single-node scenes recentre that node onto the origin, so its pivot is
