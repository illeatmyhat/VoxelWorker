...
/// [`NodeContent::Instance`] points at — reuse by reference. `Scene::add_definition` /
/// `Scene::make_definition_from_node` mint them and `Scene::add_instance` places
/// references to them (the village-of-reused-houses case).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct DefId(pub u32);

/// A **process-stable node identity**. Minted monotonically from a document-owned
/// counter ([`Scene::next_node_id`]) and durable across structural edits + undo, unlike
/// the positional [`NodePath`] (which invalidates on every add/delete/reorder).
/// `NodeId(0)` is the reserved **unassigned** sentinel a freshly-constructed [`Node`]
/// carries until [`Scene::ensure_node_ids`] mints it a real id on the load/normalization
/// path; real ids start at `1`.
///
/// The id is the identity of record: the workspace selection, the structural edit ops,
/// and the `Intent`/`Command` boundary all key on it, while `NodePath` is an ephemeral
/// render/UI projection derived on demand (see its own doc below).
#[derive(
...
/// expanded under — the two facts the picked-node rule is decided from.
///
...
impl LeafOrigin {
    ...
    /// The node a viewport pick on this body selects: the body's own node, unless it was
    /// expanded from a definition — an instance picks as ITSELF, never into its
    /// definition. Selecting a node inside a definition would address geometry shared by
    /// every other placement of it.
    pub fn picked_node(self) -> NodeId {
        self.instance_host.unwrap_or(self.node)
    }
}

/// The reserved identity of the always-present **root part**: the concrete
/// container node whose children are the scene's top-level nodes
/// ([`Scene::roots`]). It is minted once, never handed to a user node (the mint
/// floor starts real ids at `2`, see [`Scene::mint_node_id`]), so a selection naming
/// `ROOT_NODE_ID` unambiguously means "the root part is selected".
...
/// A path to a node anywhere in the **top-level assembly**.
///
/// The path is a list of child indices walked from [`Scene::roots`] down through
/// [`NodeContent::Group`] children: a single element `[i]` selects the
...
/// not addressable by a `NodePath` (a definition is edited via its instances'
/// shared body). The path therefore never descends through an `Instance`.
// `NodePath` is a purely EPHEMERAL render/UI tree projection — produced on demand by
// `path_of`/`tree_rows` and consumed within a frame by the renderer + gizmo/extent
// math. It is never stored on any type, held across frames, or serialized (identity,
// selection and storage are all `NodeId`), so it derives no
...
/// How a node combines with the nodes resolved before it. Composition is an ordered
/// document-order fold: within a scope, each node folds into the result accumulated
/// by the nodes BEFORE it under its own `CombineOp` — no operand targeting, ever.
/// Geometry is protected by *placement* (after the cutter), never by per-operation
/// target selection.
...
pub enum CombineOp {
    ...
    #[default]
    Union,
    /// Subtractive: an **occupancy-only mask** — the node's body REMOVES occupancy
    /// from everything accumulated before it among its siblings. It never stamps
    /// material; surviving cells keep the material they already had.
    Subtract,
    /// Intersective: an **occupancy-only mask** — the accumulated result KEEPS ONLY
    /// the cells the node's body also occupies; every accumulated cell OUTSIDE the
    /// body dies, including cells far outside the node's own AABB. Like `Subtract` it
    /// never stamps material (surviving cells keep their accumulated material), and
    /// intersecting the EMPTY accumulator (fold start) yields empty — predictable per
    /// the ordering law.
    Intersect,
    /// Raise or recess the accumulated surface WITHIN the node's footprint.
    ///
    ...
    /// targeting, which the ordering law admits no exception to — so this cannot decompose
...
/// Per-node grid display settings. Each grid type a node can show is gated by a
/// scene-wide master ANDed with the node's own flag; these are the per-node flags. All
/// default **off** — a freshly-added object carries no grids until the user turns them
/// on. The scene-wide masters live on [`Scene`] (`master_*`).
///
/// `voxel_grid_on_faces` reaches the resolve: it ORs the on-face-grid bit into the
/// leaf's stamped material id, so it travels with each voxel through chunk bucketing
/// (see `Scene::for_each_leaf`). `block_lattice` drives the per-object lattice extent
/// (`Scene::node_block_lattice_box_recentered`) the renderer draws from.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
pub struct NodeGrids {
    /// The on-face voxel grid overlay.
    #[serde(default)]
    pub voxel_grid_on_faces: bool,
    /// The per-object block lattice.
    #[serde(default)]
    pub block_lattice: bool,
    /// The per-object floor grid.
    #[serde(default)]
    pub floor_grid: bool,
}
...
pub struct Node {
    /// Process-stable identity, minted by [`Scene::ensure_node_ids`]. `NodeId(0)` (the
    /// default) until minted. This IS the identity of record: selection, the edit ops
    /// and the `Intent`/`Command` boundary all key on it.
    #[serde(default)]
    pub id: NodeId,
    /// Human-readable name.
    #[serde(default)]
    ...
    /// How this node folds into the result accumulated before it in its scope. For a
    /// leaf, the leaf's own role; for a `Group` (and, resolver-side, an `Instance`),
    /// the operation the scope's PRE-COMPOSED body folds under.
    #[serde(default)]
    pub operation: CombineOp,
    /// How far this node's body is DILATED before it folds.
    ///
    ...
    /// which is how a deliberate gap between chiseled pieces is authored.
    ///
    /// It is a [`Measurement`](parametric::units::Measurement), never an integer voxel
    /// count, so `"1/4 block"` survives a density change as the authored intent rather
    /// than as a stale derived number — the frame is carried, never re-derived.
    ///
    ...
    ///
    /// Zero for a node whose producer has no field at all: outset is barred there, not
    /// fabricated from a distance the producer cannot supply.
    ///
    /// # On a scope, it dilates the COMPOSED body
    ///
    /// A Part (`Group`) or a sealed `Instance` body pre-composes its children into one
    /// body, and its outset dilates THAT — not each member separately. The walk hands
    /// such a scope to the fold as a single
    ...
    pub enabled: bool,
    /// Per-node grid display settings. Absent from a config, they deserialize all-off.
    #[serde(default)]
    pub grids: NodeGrids,
    /// What the node is.
...
/// A node missing its `enabled` flag in a config defaults to enabled.
...
/// A **by-value node-tree spec** for terse construction.
///
/// [`NodeContent::Group`] stores a `Vec<NodeId>` (ids into the scene
/// [`arena`](Scene::arena)) rather than owning its children, so a caller cannot write
/// `Group(vec![child_node])` to build a subtree by value. `NodeBuilder` supplies that
/// ergonomic: a leaf carries its [`Node`] directly; a [`NodeBuilder::group`] carries
/// the by-value `Node`s/sub-builders of its children, which [`Scene::from_nodes`] /
/// [`Scene::add_definition`] flatten into the arena (minting ids depth-first, building
/// each Group's id-spine) at construction time. A plain [`Node`] converts in via
/// [`From`], so flat fixtures stay `vec![node_a, node_b]`.
...
/// `Scene::add_definition` / `Scene::make_definition_from_node` mint one and
/// `Scene::add_instance` places a reference to it, so the same definition placed by
/// N instances is visited N times at resolve (the village-of-reused-houses case).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct AssemblyDef {
    ...
    #[serde(default)]
    pub name: String,
    /// The nodes that make up this assembly: an ordered spine of child [`NodeId`]s;
    /// the child `Node`s live in the scene-wide [`Scene::arena`]. The `Vec` order is
    /// document order.
    #[serde(default)]
    pub children: Vec<NodeId>,
    /// Whether this definition is a **fixture**: a fixture does NOT pre-compose — its
    /// children splice into the HOSTING scope's
    ...
    /// [`Scene::walk_nodes`]). `serde(default)`: a document without the field loads
    /// sealed.
    #[serde(default)]
    pub fixture: bool,
}

/// A **process-stable Point identity**. Minted from the same document-owned counter as
/// [`NodeId`] ([`Scene::next_node_id`]) so the undo machinery's counter rewind covers
/// both kinds with one field; the two never collide because they are different types
/// keying different collections. `PointId(0)` is the unassigned sentinel a deserialized
/// Point without one carries until [`Scene::ensure_node_ids`] mints it a real id.
/// **Never a `Vec` index** — an index shifts on `RemovePoint` and silently re-points at
/// a *different* Point, which an existence check cannot catch.
#[derive(
    Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Default, Serialize, Deserialize,
)]
pub struct PointId(pub u64);

/// A world-anchored **reference element**: a named point in the world-block lattice
/// that carries optional reference planes (ground / front / side) and axis lines.
/// Distinct from the per-selection transform gizmo — a Point is a persistent
/// annotation in world space.
///
...
///
/// Points are rendered as a camera-relative overlay, rebuilt every frame from
/// this list — a hidden/shown Point or a plane/axis toggle takes effect immediately,
/// with no re-resolve.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct Point {
    /// Stable identity — what the workspace selection keys on, never the `Vec` slot.
    /// The index-keyed Point *intents* stay sound (they are transient and undo replay
    /// is deterministic); identity is only needed where a reference outlives mutations
    /// it cannot see.
    #[serde(default)]
    pub id: PointId,
    ...
    /// Sub-block offset in voxels. The field exists so sub-block placement is a data
    ...
    #[serde(default)]
    pub plane_yz: bool,
    /// Whether the +X axis line shows. Default **true**. X, Y and Z are independently
    /// toggleable, never one combined `axes` flag.
    #[serde(default = "default_true_bool")]
    pub axis_x: bool,
