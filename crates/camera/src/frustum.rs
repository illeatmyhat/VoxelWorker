//! View-frustum culling: plane extraction and the positive-vertex AABB test.
//!
//! [`Frustum::from_view_projection`] recovers the six clip-space frustum planes
//! directly from a combined `view_projection` matrix, and [`Frustum::intersects_aabb`]
//! tests a world-space axis-aligned box against them. A chunk is drawn only when its
//! box intersects the frustum, so at small scale every chunk is visible (identical
//! output) while a large scene viewed partially off-screen draws only the chunks on
//! screen. Pure CPU math — no GPU types — so it is unit-testable without a device.
//!
//! The box under test is substrate's [`RealAabb`]: the **closed** continuous f32 box
//! (touching boxes intersect), co-located in substrate beside its half-open integer
//! twin `LatticeAabb` — see substrate's `aabb` module for the side-by-side contrast.
//! The closed convention is exactly the conservative bias culling needs: a chunk
//! whose face lies ON a frustum plane must still be drawn.
//!
//! Cite: the plane extraction is Gribb & Hartmann, "Fast Extraction of Viewing
//! Frustum Planes from the World-View-Projection Matrix" (2001); the box test is the
//! **positive-vertex** (p-vertex / n-vertex) test from Ericson, *Real-Time Collision
//! Detection* (2005) §5.2.3 — for each plane only the box corner farthest along the
//! inward normal need be tested, and rejecting on that corner never yields a false
...
mod tests {
    ...
    fn unit_box_at(center: Vec3) -> RealAabb {
        RealAabb {
