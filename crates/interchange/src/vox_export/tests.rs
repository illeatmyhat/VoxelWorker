...
fn atomic_write_leaves_no_temp_and_temp_is_a_dir_sibling() {
    ...
    let dir = std::env::temp_dir().join(format!("voxel_worker_write_test_{}", std::process::id()));
    ...
    assert_eq!(
        temp.parent(),
        Some(dir.as_path()),
        "temp is a sibling of the final file"
    );
    assert!(
        temp.file_name()
            .and_then(|n| n.to_str())
...
fn vox_splits_models_over_256() {
    ...
    let export = VoxExport::from_grid(
        &grid,
        VoxExport::block_palette_from_active(MaterialChoice::Stone, [200, 200, 200, 255]),
    );
...
fn streamed_vox_export_equals_dense_for_flat_and_odd_shapes() {
    for kind in [ShapeKind::Cylinder, ShapeKind::Sphere, ShapeKind::Torus] {
        for size in [[5u32, 1, 5], [3, 1, 3], [5, 3, 5], [1, 1, 1]] {
            ...
            assert_streamed_vox_export_equals_dense(&scene, 16, &format!("{kind:?} {size:?}"));
        }
    }
}
...
fn streamed_vox_export_dissolves_6m_cap_on_large_solid() {
    ...
    assert_eq!(
        total, region_voxels,
        "every voxel must survive the 256-split tiling"
    );
    for model in &parsed.models {
        assert!(model.size.x <= 256 && model.size.y <= 256 && model.size.z <= 256);
    }
