//! The **brick display path**: the brick-field BUILD, its clip-map pyramid,
...
//!
//! The build packs the two-layer boundary set into a sorted [`BrickRecord`]
//! array (keyed by a packed absolute world-block key — the frame is world-fixed and
//! carried, never re-derived) plus an R8 3D texture atlas of sculpted-brick occupancy,
//! **surface-only** (interior elision):
//!
...
//!
//! The **brick granule is ONE BLOCK**: the brick edge is
...
//!
//! [`BrickRaymarchRenderer`] is a fullscreen pass that walks a block-space DDA
...
//!   form (the residency-miss contract) — degraded-but-correct, never skipped.
...
//!   `cuboid.wgsl`.
//!
//! The sink is a **display derivation**: records + atlas are built from CPU
...
//! mesh stays the headless/no-GPU fallback.
...
// Sibling submodules reach each other through `use super::*` + these globs.
pub use atlas::*;
pub use build::*;
pub use clipmap::*;
pub use cpu_march::*;
