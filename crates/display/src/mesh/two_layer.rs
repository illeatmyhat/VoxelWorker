...
pub(crate) enum NeighborFace {
    /// The whole face is solid (a coarse-solid neighbor).
    Solid,
...
/// in-plane (a, b) ordering both the neighbor-face bitmap and the apron fill index by.
#[inline]
...
/// analog) rather than a densified neighbor apron.
///
...
pub(crate) fn build_two_layer_chunk_meshes_filtered(
    ...
    // Unwrap the carried frame at the per-chunk rebase arithmetic below (`chunk_min_recentered`).
    ...
    let band_lo_recentered = band.band_min as i64 - half_z;
    let band_hi_recentered = (band.band_max as i64).saturating_sub(half_z);
    // Whether a recentered-frame voxel-Z index is inside the band.
    let z_in_band = |recentered_z: i64| -> bool {
        if !band_active {
            return true;
        }
        recentered_z >= band_lo_recentered && recentered_z <= band_hi_recentered
    };
    let chunk_extent_voxels = (CHUNK_BLOCKS * density) as i64;

    // A lookup of every covering chunk by coord so a block can consult its neighbor's
    ...
    // 1-voxel layer that abuts a neighboring block across that face. A coarse-solid block
    // is fully solid (the seam-flag fast path — no densification); an air block fully air; a
    // boundary block expands ITS cuboids' face layer per cell. This is the exact neighbor
    ...
    let face_cells_at = |abs_block: [i64; 3], axis: usize, side: usize| -> NeighborFace {
        let chunk_blocks = CHUNK_BLOCKS as i64;
        let chunk_coord = [
            abs_block[0].div_euclid(chunk_blocks) as i32,
            abs_block[1].div_euclid(chunk_blocks) as i32,
    ...
    let meshes: Vec<CuboidChunkMesh> = chunks
        ...
        .filter_map(|(chunk_coord, chunk)| {
            ...
            let chunk_min_recentered = [
                chunk_coord[0] as i64 * chunk_extent_voxels - recenter_voxels[0],
                chunk_coord[1] as i64 * chunk_extent_voxels - recenter_voxels[1],
                chunk_coord[2] as i64 * chunk_extent_voxels - recenter_voxels[2],
            ];
            ...
            for block_z in 0..CHUNK_BLOCKS {
                for block_y in 0..CHUNK_BLOCKS {
                    for block_x in 0..CHUNK_BLOCKS {
                        ...
                        // The block's low voxel corner in the recentered frame.
                        let block_low_recentered = [
                            chunk_min_recentered[0] + block_x as i64 * block_extent,
                            chunk_min_recentered[1] + block_y as i64 * block_extent,
                        ...
                        let fully_out_of_band_z =
                            block_hi_z < band_lo_recentered || block_lo_z > band_hi_recentered;
                        let block_hi = [
                            block_low_recentered[0] + block_extent - 1,
                            block_low_recentered[1] + block_extent - 1,
                            block_hi_z,
                        ];
                        ...
                        match route {
                            ...
                            BlockRoute::Fast => {
                                if let Some(block_id) = chunk.coarse_block(block) {
                                    ...
                                    // block-local apron filled PER CELL from the NEIGHBOR blocks' face
