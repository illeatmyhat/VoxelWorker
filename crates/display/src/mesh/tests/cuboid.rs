...
/// until they are rebuilt. Nothing rescaled — the render origin is the midpoint of the composite
/// extent, so growing a shape moves it, the camera is compensated at dispatch, and the mesh goes
/// on standing in the frame it was emitted in until the next build lands. For that window the
/// camera is somewhere the vertices are not, and the whole model slides by the difference.
///
...
fn a_mesh_baked_in_an_older_frame_still_images_where_it_belongs() {
    ...
    let clip_of = |world: glam::Vec3, baked: RecenterVoxels, current: RecenterVoxels| {
        ...
        camera_walked_into_the_baked_frame(camera_of(current), baked, current) * vertex.extend(1.0)
    };
    ...
    assert_eq!(
        camera_walked_into_the_baked_frame(camera, opening, opening).to_cols_array(),
