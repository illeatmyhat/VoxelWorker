...
/// recentered frame; `abs_block` its absolute block coord (to look up neighbors).
#[allow(clippy::too_many_arguments)]
pub(crate) fn emit_coarse_block_box(
    block_id: voxel_core::core_geom::BlockId,
    overlay: bool,
    density: u32,
    block_low_recentered: [i64; 3],
    abs_block: [i64; 3],
    ...
    // The box spans the block: world min corner = block_low_recentered, far plane = + density.
    let lo = [
        block_low_recentered[0] as f32,
        block_low_recentered[1] as f32,
        block_low_recentered[2] as f32,
    ];
    let hi = [
        (block_low_recentered[0] + density as i64) as f32,
        (block_low_recentered[1] + density as i64) as f32,
        (block_low_recentered[2] + density as i64) as f32,
    ];
    ...
    for face in &FACE_TEMPLATES {
        // The face's axis + side, and the neighbor block across it.
        ...
        // The neighbor's MATCHING face is on the same axis, OPPOSITE side. If it is fully
        ...
        let neighbor_face_solid = face_solidity_at(neighbor).face_is_solid(axis, 1 - side);
        if neighbor_face_solid {
            continue;
        }
...
pub(crate) fn emit_boundary_block_cuboids(
    ...
    block_low_recentered: [i64; 3],
    abs_block: [i64; 3],
    face_cells_at: &dyn Fn([i64; 3], usize, usize) -> NeighborFace,
    vertices: &mut Vec<CuboidVertex>,
    indices: &mut Vec<u32>,
    indices_overlay: &mut Vec<u32>,
    aabb: &mut Aabb,
    ...
    // 1-voxel border is the apron. `face_is_exposed` then tests the neighbor cell exactly.
    ...
    // Apron border: each of the 6 outer planes is filled PER CELL from the neighbor block's
    ...
    for (axis, side, delta) in [
        ...
        // The neighbor's MATCHING face is on the same axis, OPPOSITE side.
        let neighbor_face = face_cells_at(neighbor, axis, 1 - side);
        if matches!(neighbor_face, NeighborFace::Air) {
        ...
        for ai in 0..d {
            for bi in 0..d {
                let solid = match &neighbor_face {
                    NeighborFace::Solid => true,
                    NeighborFace::Cells(cells) => cells[(bi * d + ai) as usize],
                    NeighborFace::Air => false,
                };
                if !solid {
                    continue;
                }
    ...
    // Region offset maps apron-local index 0 to the recentered frame: the block's low voxel
    // is apron-local +1, so apron-local 0 sits at `block_low_recentered - 1`.
    let region_offset = [
        (block_low_recentered[0] - 1) as f32,
        (block_low_recentered[1] - 1) as f32,
        (block_low_recentered[2] - 1) as f32,
    ];
    for cuboid in &geometry.cuboids {
...
/// `block_low_recentered_z + vz` for the band test — masking out-of-band voxels to air on BOTH
/// the meshed interior and the neighbor apron, exactly as the dense banded path masks apron.
///
/// Writes only cells whose apron-local index lands inside `region.extent` (a neighbor block
...
pub(crate) fn stamp_block_into_region_banded(
    ...
    let stamp = |vx: u32, vy: u32, vz: u32, key: u16, out_region: &mut VoxelRegion| {
        let recentered = [
            block_low_recentered[0] + vx as i64,
            block_low_recentered[1] + vy as i64,
            block_low_recentered[2] + vz as i64,
        ];
        if !voxel_meshed(recentered, z_in_band, clip) {
            return;
        }
...
pub(crate) fn emit_block_banded(
    ...
    let mut apron = interior; // reuse as the exposure region; add the neighbor border below.

    // Apron border: each of the 6 neighbor blocks' abutting face, band-clipped. A neighbor
    ...
    // origin). Only the single border layer of each neighbor falls inside the apron extent.
    for (delta, dst_lo) in [
        ...
        let neighbor_low_recentered = [
            block_low_recentered[0] + delta[0] * density as i64,
            block_low_recentered[1] + delta[1] * density as i64,
            block_low_recentered[2] + delta[2] * density as i64,
        ];
        stamp_block_into_region_banded(
            chunk_by_coord,
            neighbor,
            density,
            neighbor_low_recentered,
            dst_lo,
            z_in_band,
            clip,
            &mut apron,
    ...
    // Region offset maps apron-local index 0 to the recentered frame: the block's low voxel is
    // apron-local +1, so apron-local 0 sits at `block_low_recentered - 1`.
    let region_offset = [
        (block_low_recentered[0] - 1) as f32,
        (block_low_recentered[1] - 1) as f32,
        (block_low_recentered[2] - 1) as f32,
