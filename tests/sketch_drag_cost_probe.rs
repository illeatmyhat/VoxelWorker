...
fn report(label: &str, made: &SketchSolid) {
    ...
    // 4. The re-resolve the shell triggers by mutating the node.
    ...
    println!(
        "{label:<20} {entities:>6} {relations:>6} {loops:>6} {clone_ms:>8.2} {move_ms:>8.2} \
         {region_ms:>8.2} {rebuild_ms:>9.2} {total:>8.2}"
    );
}
...
fn sketch_drag_frame_cost_by_population() {
    println!(
        "\n{:<20} {:>6} {:>6} {:>6} {:>8} {:>8} {:>8} {:>9} {:>8}",
        "drawing", "points", "rels", "loops", "clone", "move", "region", "rebuild", "total"
    );
    println!("{}", "-".repeat(84));
