...
| **Per-voxel material modulation** (ADR 0001 step 3) | Per-instance `material_id` indexes `material_base_colors`; fragment multiplies lit color by the material's relative base color. | `material_id` rides **per cuboid/quad** (a box is one material by construction — see Meshing). Carried as a flat per-vertex attribute (constant across the merged face). Modulation toggle + relative-base-color uniform survive as-is. A merged face must **never straddle two materials** — the mesher splits on material change. |
| **Layer-range band clip** (#12 scrubber) | Fragment recovers the voxel's Y layer from the instance center and discards outside `[band_min, band_max]`. | The band clip becomes **per-fragment from the absolute Y voxel layer** (already a varying candidate). A merged quad spanning the band edge must clip mid-quad → keep the **fragment discard** on interpolated absolute-Y (do NOT bake the band into the mesh, or scrubbing forces a re-mesh every frame). Cheaper alternative: clip whole chunks/cuboids by AABB, fragment-discard only the boundary chunk. |
| **`--debug-faces` mode** | Cull-off pipeline + shader colors by outward normal and stripes back-faces to expose winding bugs. | The mesher must emit **correctly wound, outward-normal faces**; debug-faces is the regression check that it does. Keep the cull-off debug pipeline variant; color-by-normal needs the face normal as a varying (already present). This mode is also the **primary golden image** (#24, deterministic). |
| **Onion-skin fog occupancy** (#12, `OnionFogRenderer`) ✅ **REWORKED to per-chunk, now DEFAULT** (2026-06-25, #28 S5a/S5b) | Resolves the grid to a **3D R8 occupancy texture** and raymarches it as a cloud; sized to grid dims, capped by max 3D texture dimension. | **DONE.** Fog reads **occupancy, not the mesh** (decoupled from meshing). Per-chunk occupancy is now the **default** path: one apron'd `R8` volume per resident chunk, packed into a single small 3D **atlas** (atlas dimension bounded by chunk COUNT, not whole-grid extent), with a metadata uniform of per-chunk world origins; the raymarch (`onion_fog_perchunk.wgsl`) marches in recentered world space and candidate-samples the owning chunk's tile. The 1-voxel apron is filled from the GLOBAL occupancy so trilinear sampling across a chunk seam is **continuous (no banding/seam lines)** — A/B vs the legacy whole-grid path is **0.0000%** on normal scenes, and it still renders fog at scale where whole-grid exceeds `max_texture_dimension_3d` and disables itself. Legacy whole-grid kept as a fallback (`--fog=wholegrid`). Locked by the `onion-fog-perchunk` golden (#24). Region-scoping the resident set is deferred to #20. |
...
- **64-bit world addressing.** `i32`→`i64` **block** coordinates + a sub-block **voxel** offset.
  `Voxel.world_position` (today `[f32; 3]`, world-centered voxel-grid coords) is **not** the storage
...
7. **E5 — Out-of-core store + palette/sparse (Step 7, #20).** 🟡 **IN PROGRESS — building blocks
   ...
   - **Wire the region-scoped consumers into the live app** ("Step 2"): route the interactive diameter
     ...
     voxel-center `.5` on export. Parity-proven; interactive-only verification.
   - **Per-chunk GPU residency + remove the monolithic assembly** ("Step 4"): make the cuboid renderer
     ...
     **[STATUS 2026-06-29] The dirty-chunk re-mesh is now CONSUMED by the live cuboid renderer (#40,
     ...
     the chunks an edit (+ its 1-voxel apron neighbors) touched via `CuboidMeshRenderer::incremental_rebuild_from_chunks`,
     driven by the apron-aware `cuboid_mesh::cuboid_incremental_plan` (the dirty set DILATED by the
     26-neighborhood — the old instanced `renderer::incremental_rebuild_plan` is apron-unaware and not
...
3. **Dirty-whole-chunk invalidation + mesh cache.** VS: "if any block in the chunk is dirty, the
   **entire chunk** is re-tessellated." → **Validates our coarse invalidation granularity
   ...
   cuboid renderer re-meshes only the dirty chunks (DILATED by the 1-voxel apron's 26-neighborhood, so
   a seam neighbor's culling stays correct) via `cuboid_mesh::cuboid_incremental_plan` +
