...
- **Layer:** BOUNDARY RULING + foundation refinement. **Extends and sharpens [ADR 0006](0006-authoring-truth-and-gpu-boundary.md)**
  (the CPU/GPU authoring-truth boundary) and generalizes the retired fog resolver (the first display sink),
...
(store unification) with a classification step and generalizes the covering-vs-non-empty distinction to a
boundary set.
...
(serves display, A/B-checked against the CPU one). The op-stack is the shared *input*;
...
- Retire the monolithic `resolve_region` whole-grid assembly; the per-chunk store stays
...
- The retired GPU fog resolver was the **first display sink**; the new evaluator generalizes its role.
...
design rule — *optimize for recompute, not render, because the world is dynamic* — is our exact situation. **The earlier
...
- **Display path — DECIDED: C, the cached sparse brick field + clip-map LOD** (benchmarked RTX 4090, 4K, 20 FPS;
  ...
  independent of scene complexity** — and it generalizes the shipped fog atlas. Measure-don't-assume
