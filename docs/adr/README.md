...
| [0048](0048-a-dimension-states-a-quantity-at-a-place.md) | A dimension states a quantity, at a place | Accepted. Extends 0035 decision 5 with the `Dimension` members that shipped after it: a diameter is a member and not a display flag because the number is authored, and an angle's arm is a TYPE — on a curve that turns it names an END, chosen by the click, because a point on the curve would be a coincidence every later solve has to keep agreeing about |
