...
| **scene browser (always open)** | the normal left scene tree stays open in sketch mode — the sketch node highlighted "editing" + the **external** parts/geometry — but **NOT** the sketch's own vertex/edge list | you must see & reach scene objects to **snap the path / on-surface profile to them** (sweep-path-snap, on-surface sketching); owner: "keep it as it has been." The seal is about **composition** (booleans don't leak, no operand targeting), not visibility — snapping to external geo captures a *coordinate value*, not a live link | owner review 2026-07-22 |
